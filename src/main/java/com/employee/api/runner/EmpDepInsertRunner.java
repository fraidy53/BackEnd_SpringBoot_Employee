...
@Component // Spring Bean으로 등록하여 애플리케이션 실행 시 자동으로 실행되도록 함
@Profile("test") // "local" 프로파일이 활성화된 경우에만 이 Runner가 실행
public class EmpDepInsertRunner implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) throws Exception {
        ...
        departmentRepository.saveAll(List.of(department1, department2, department3)); // 한꺼번에 저장(saveAll)
